...
fn main() {
    ...
    let final_sum = dive_part_two("./src/input.txt");
    println!("part two: {}", final_sum);
}
...
fn dive_part_two(filename: &str) -> u32 {
    let sum = if let Ok(lines) = read_lines(filename) {
        let instructions: Vec<Instruction> = lines
            .map(|l| l.unwrap())
            .map(|l| {
        ...
        for instr in instructions {
            match instr.direction {
                ...
                Directions::DOWN => aim += instr.value,
                Directions::UP => aim -= instr.value,
            }
        }
        horizontal * depth
    } else {
